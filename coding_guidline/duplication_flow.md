...
言い換えると、デモサイト環境を別サーバーへ複製するためのマニュアルです。  
...
`wp_2`のように番号がついているデータは、子サイトのDBテーブルです。（例； [https://order1a.wpt-demo.com/copytest](https://order1a.wpt-demo.com/copytest)）  
...
## 3. 複製先のDBへエクスポートしたSQLをインポートする

インポート前に複製先のサーバーにて空のDBを新規作成しておきます。  
インポート時はデフォルト設定のままでインポートを行います。


### Tips：DB名の命名規則について
もし既存のDBがあり、かつ規則性のあるDB名であれば、それに合わせて作成するのがベターでしょう。
...
管理のしやすさ、名前だけで何のDBか判断できるかが基準になると思います。  
